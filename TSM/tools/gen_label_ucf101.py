...
import fnmatch
import random
root = r"C:/Code/Gesture_Vision/TSM/data/img/"      # 抽帧后的图片存放目录文件夹，用于写到txt文件中在构建数据集的时候读取

def parse_ucf_splits():
    class_ind = [x.strip().split() for x in open(r'C:\Code\Gesture_Vision\TSM\data\label\classInd.txt')]       # 类别txt
    ...
    splits = []
    train_list = [line2rec(x) for x in open(r'C:\Code\Gesture_Vision\TSM\data\label\trainlist01.txt')]       # 训练集txt
    test_list = [line2rec(x) for x in open(r'C:\Code\Gesture_Vision\TSM\data\label\testlist01.txt')]          # 测试集txt
...
f_info = parse_directory(r"C:\Code\Gesture_Vision\TSM\data\img")    # 存放抽帧后的图片

out_path = r"C:\Code\Gesture_Vision\TSM\data\label"      # 标签路径
dataset = "ucf101"

# for i in range(max(1,len(spl))):
# for i in range(1):
lists = build_split_list(spl,f_info,0)
...
open(os.path.join(out_path, 'train_split.txt'), 'w').writelines(lists[0][0])
open(os.path.join(out_path, 'val_split.txt'), 'w').writelines(lists[0][1])
